# DOMINO BRAIN — Case History per metodo
> Versione 1.0 — Maggio 2026. Mappa retrospettivo-canonica: per ciascun case history elenca i metodi/prodotti del **catalogo 2026** (Core Sprint!, Design Sprint! con le sue varianti, Brain & Identity Design Sprint!, Build Sprint!, Trainstorming!, Preventivo Emozionale, Audit tattico) che corrispondono al lavoro effettivamente svolto. Da usare per la pagina di spiegazione metodi (Figma `Domino_DesignSystem`) e per orientare la narrazione commerciale dei case.

**Link rapidi:** [Identità](01_domino_identita.md) · [Servizi](02_domino_servizi.md) · [Metodi](03_domino_metodi.md) · [Case history](04_domino_case_history.md) · [Settori](05_domino_settori.md) · [Referenze](06_domino_referenze.md)

---

## Avvertenza metodologica

La mappatura è **retrospettivo-canonica**: rilegge case storici alla luce del catalogo 2026 a quattro Sprint! (Core → Design → Build → Trainstorming!) più Preventivo Emozionale e Audit tattico. Solo alcuni case hanno percorso documentato come tale nel Brain:

- **CIA — Confederazione Italiana Agricoltori** *(Core Sprint! + 2 Design Sprint! — documentato)*
- **Poltrona Frau In Motion** *(Core Sprint! + Design Sprint! + Build Sprint! — documentato)*
- **Links Foundation — Nuova Intranet** *(Design Sprint! + Agile — documentato)*
- **Stellantis — Employee Portal** *(letto come "transition program" nella scheda I5.0 del Brain)*

Tutti gli altri sono assegnati per affinità con il pattern di lavoro descritto. Utile come griglia di spiegazione metodi, da non spacciare in pitch come storico letterale del cliente specifico.

---

## Percorsi a Sprint! — documentati

- **CIA — Confederazione Italiana Agricoltori** → Core Sprint! · Design Sprint! ×2 *(documentato)*
- **Poltrona Frau In Motion** → Core Sprint! · Design Sprint! · Build Sprint! *(documentato)*

---

## Automotive

- **IVECO — iveco.com** (IKA 2024) → Design Sprint! (Website) · Build Sprint!
- **IVECO — MyIVECO App** → Design Sprint! (CX) · Build Sprint!
- **IVECO — TCO Calculator** → Design Sprint! (Service) · Build Sprint!
- **IVECO — eDailyAdvisor** → Design Sprint! (Service) · Build Sprint!
- **IVECO — Training con AI Video** → Trainstorming! · Build Sprint!
- **Stellantis — Employee Portal** → Design Sprint! (Intranet) · Trainstorming! · Build Sprint!
- **Stellantis — Electrification Days** → Design Sprint! · Trainstorming! · Build Sprint!
- **Stellantis — D&I People Stories** → Design Sprint! · Trainstorming! · Build Sprint!
- **Fiat — Sito internazionale EMEA** → Design Sprint! (Website) · Build Sprint!
- **Fiat Professional (Ducato) — App E-Product** → Design Sprint! (Service) · Build Sprint! · Preventivo Emozionale *(logica analoga)*
- **Jeep — Community digitale** → Design Sprint! (Service) · Build Sprint!
- **CIFA** → Design Sprint! · Build Sprint!
- **Case IH — Lead generation Europa + Brand Awareness** → Design Sprint! (Digital Marketing) · Build Sprint!
- **Case IH — IKA 2024 Innovation Technology** → Design Sprint! · Build Sprint! *(AI in campagna lead gen)*
- **Case IH — Sito con Personas per 18 mercati** → Design Sprint! (Website) · Build Sprint!

---

## B2B Industriale

- **Comau — Brand Identity** → Design Sprint! (Brand) · Build Sprint!
- **Comau+ — Marketplace** → Design Sprint! (Service) · Build Sprint!
- **Bitron — CX B2B & Web di Gruppo** → Design Sprint! (CX) · Build Sprint! · Audit tattico SEO
- **Rollon — "Il prodotto diventa esperienza"** (IKA 2023) → Design Sprint! (Website/CX) · Build Sprint! · GEO/SEO
- **Megadyne — Digital B2B** → Design Sprint! · Build Sprint!
- **Contship Italia — Digital growth** → Build Sprint! · Audit Digital Marketing
- **Danieli Automation — Design System** → Brain & Identity Design Sprint! *(focus design system)* · Build Sprint!
- **IPI Agency — Real Estate** → Build Sprint!
- **IPI — SFERA Intranet** → Design Sprint! (Intranet) · Build Sprint!
- **IPI — Internal Communication Onboarding** → Trainstorming! · Build Sprint!
- **BANCOMAT — Da back-office a engagement** → Design Sprint! · Build Sprint!
- **Links Foundation — Nuova Intranet** → Design Sprint! (Intranet) · Build Sprint! *(Design Sprint! documentato)*
- **Demak Group — Da fornitore a leader B2B** → Core Sprint! · Design Sprint! (Brand) · Build Sprint! · Audit tattico SEO/GEO
- **Frascold — SEO** → Audit tattico SEO/GEO · Build Sprint!
- **Veritas — Scoasse** → Build Sprint!
- **Pipein — Da startup ad azienda** → Core Sprint! · Design Sprint! (Brand) · Build Sprint!
- **Gruppo De Eccher** → Build Sprint!

---

## Digital Marketing & SEO/GEO

- **Masi Agricola — Sito + SEO + eCommerce** → Design Sprint! (Website) · Build Sprint! · Audit tattico SEO/GEO
- **ENIT — Promozione Italia nel mondo** → Design Sprint! (Digital Marketing) · Build Sprint!

---

## CX / eCommerce / Finance

- **Arca SGR — Advisory Assistant** → Design Sprint! (Service) · Build Sprint!
- **Costa Crociere — Preventivo Emozionale** → Preventivo Emozionale · Build Sprint!

---

## AI & Innovazione

- **BR-Uno — Intelligent Assistant Anti-bullying** → Brain & Identity Design Sprint! *(focus brain conversazionale)* · Build Sprint!
- **GEDI + Stellantis — Alfabeto del Futuro** → Design Sprint! (Brand/Service) · Build Sprint!

---

## Turismo & Cultura

- **Biennale di Venezia — Sito + eCommerce** → Design Sprint! (Website) · Build Sprint!
- **Fondazione Torino Musei — Rete museale** → Design Sprint! (CX) · Build Sprint!
- **Museo Nazionale del Cinema** → Build Sprint!
- **Alpitour — Digital Marketing & Sales** → Audit tattico Digital Marketing · Build Sprint!
- **Langhe-Monferrato-Roero** → Build Sprint!
- **Distretto Turistico dei Laghi** → Build Sprint!

---

## Salute & Beauty

- **Lierac + Phyto (Ales Group) — CX prima del prodotto** → Design Sprint! (CX) · Build Sprint!
- **Ospedale dell'Angelo (Venezia-Mestre)** → Design Sprint! (CX) · Build Sprint!
- **Affidea Italia** → Design Sprint! (CX) · Build Sprint!
- **LARC — Digital innovation in healthcare** → Design Sprint! (Website) · Build Sprint!

---

## Collegamenti al Brain

- [03_domino_metodi.md](03_domino_metodi.md) — schede complete dei metodi (Core Sprint!, Design Sprint! + varianti, Brain & Identity Design Sprint!, Build Sprint!, Trainstorming!, Preventivo Emozionale)
- [04_domino_case_history.md](04_domino_case_history.md) — schede dei case per cliente con KPI e categoria
- [case_cia_percorso_sprint.md](case_cia_percorso_sprint.md) — scheda completa CIA
- [case_poltronafrau_inmotion_percorso_sprint.md](case_poltronafrau_inmotion_percorso_sprint.md) — scheda completa Poltrona Frau In Motion
