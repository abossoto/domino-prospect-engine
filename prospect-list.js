...
// ─── BRAIN LOADER (tutti i file .md inclusi i GTM verticali) ────────────────
function loadBrain() {
  const files = [
...
// ─── RESEARCH SYSTEM (come array con cache_control) ──────────────────────────
// Il system prompt del research agent è statico → viene cachato dopo la prima chiamata.
const RESEARCH_SYSTEM = [
  {
    type: 'text',
    text: `Sei un analista di intelligence commerciale senior per Domino, agenzia CX italiana.
...
4. LINKEDIN
   - Cerca prima con Google: "[azienda] CEO site:linkedin.com/in", "[azienda] CMO OR Marketing Director site:linkedin.com/in"
   - Non accedere direttamente a linkedin.com — usa risultati Google con snippet LinkedIn
...
## ⚠️ DATI NON TROVATI (obbligatoria — guida la ricerca manuale del commerciale)`,
    cache_control: { type: 'ephemeral' },  // ← cachato: non cambia mai tra chiamate
  }
];
...
- DECK: slide 1 su contesto settore, slide 2 sulla sfida strategica, slide 3 su come altre
  ...
  (nessun commitment).
- WORKFLOW: 3 touch in 3 settimane. Touch 2 = condividi contenuto rilevante sul settore,
  non pitch Domino.`,
...
- MAIL: aggancia con pain point specifico del settore del prospect. Cita 1-2 clienti Domino
  dello stesso settore con metrica. CTA = call per approfondire quel tema.
...
- MAIL: descrivi il loro problema specifico nel loro linguaggio (non quello dell'agenzia).
  Cita metrica di business (non tecnica). CTA = Design Sprint o call operativa di 30 min.
- DECK: slide 1 il problema (loro lingua), slide 2 perché è difficile (insight non ovvio),
  slide 3-4 case con numeri, slide 5 Design Sprint come prossimo passo a basso rischio.
- WORKFLOW: 4 touch. Touch 3 = proponi esplicitamente un Design Sprint con descrizione
  e costo indicativo.`,
...
- DECK: slide 1 analisi loro presenza digitale attuale, slide 2 benchmark di settore,
  ...
  slide 5 quick win possibile nelle prime 4 settimane.
...
- DECK: slide 1 comprensione del loro brief (dimostra ascolto), slide 2 proposta specifica,
  slide 3 metodo (come si lavora dal giorno 1), slide 4 case più affine,
  slide 5 investimento indicativo e timeline.
...
// ─── BUILD GENERATION SYSTEM (brain cachato, GTM instructions no) ────────────
// Il brain (11 file .md) non cambia mai → cache_control su quel blocco.
// Le istruzioni GTM variano per layer/motion → nessun cache_control su quel blocco.
function buildGenerationSystem(brain, layer, motion) {
  ...
  return [
    {
      type: 'text',
      text: brain,  // ← gli 11 file .md concatenati — cachato perché statico
      cache_control: { type: 'ephemeral' },
    },
    {
      type: 'text',
      text: `Sei il generatore di materiali sales di Domino. Ricevi un report di intelligence e produci output personalizzati.
...
Restituisci ESCLUSIVAMENTE JSON puro. Zero testo. Zero markdown. Zero backtick.`,
      // nessun cache_control → non cachato (varia per layer/motion)
    },
  ];
}
...
async function runResearch(prospect, note) {
  ...
  while (data.stop_reason === 'tool_use' && i < 20) {
    ...
    const feedback =
      i < 8  ? 'Continua — cerca ancora LinkedIn per nomi manager (usa Google con site:linkedin.com/in) e Cerved per dati finanziari.' :
