        ...
        {mode === 'analizza' && <>
          ...
          <Card style={{ marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '14px' }}>
              <Btn onClick={analyze} disabled={loading || !input.trim()} style={{ minWidth: '160px', fontSize: '14px' }}>
          ...
          {result && p && (
            <>
              <div style={{ background: '#080808', border: `1px solid ${C.border}`, borderRadius: '12px', padding: '12px 18px', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '14px', flexWrap: 'wrap' }}>
                ...
                <div style={{ width: '1px', height: '30px', background: C.border }} />
                {/* GTM badge in result header */}
                <div>
                  <Label>GTM</Label>
                  <div style={{ fontSize: '11px', color: '#aaa', display: 'flex', gap: '6px', marginTop: '2px' }}>
                    {(() => { const l = GTM_LAYERS.find(x => x.id === gtmLayer); return l ? <span style={{ color: l.color, fontWeight: 700 }}>{l.label}</span> : null; })()}
                    <span style={{ color: C.muted }}>·</span>
                    {(() => { const m = GTM_MOTIONS.find(x => x.id === gtmMotion); return m ? <span>{m.label}</span> : null; })()}
                  </div>
                </div>
